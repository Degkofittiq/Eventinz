    ...
    <form method="POST" action="{{ route('admin.update.subscriptionplan', $subscriptionFound->id) }}" enctype="multipart/form-data">
      ...
      <div class="card-body">
        ...
        <div class="form-group">
          ...
          <input value="{{ $subscriptionFound->vendorType  == 1 ?? " " }}" name="vendor_service_types_id" id="vendor_service_types_id" class="form-control  @error('vendor_service_types_id') is-invalid @enderror">
          @error('vendor_service_types_id') <p> {{ $message }} </p> @enderror
        </div>
        <div class="form-group">
          <label for="price">Price($) <span style="color: red"><strong>*</strong></span></label>
          <input value="{{ $subscriptionFound->price ?? " " }}" name="price" id="price" class="form-control  @error('price') is-invalid @enderror">
          @error('price') <p> {{ $message }} </p> @enderror
        </div>
        <div class="form-group">
          <label for="duration">Duration <span style="color: red"><strong>*</strong></span></label>
          <input value="{{ $subscriptionFound->duration ?? " " }}" name="duration" id="duration" class="form-control  @error('duration') is-invalid @enderror">
